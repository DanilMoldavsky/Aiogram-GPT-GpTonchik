...
def set_text(
    ...
    """
    Сохраняет текст в словарь

    :param telegram_id: ID юзера в Telegram
    :param text: текст
    :param context: (опционально) контекст
    """
    data.setdefault(telegram_id, dict())
    data[telegram_id]["text"] = text
    if context:
        data[telegram_id]["context"] = context
...
def mess_edit_set(
    ...
    data.setdefault(telegram_id, dict())
    data[telegram_id]["mess_edit_set"] = id_message
